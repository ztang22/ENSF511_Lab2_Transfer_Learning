...
#filelist=glob.glob('/home/ubuntu/images/*.jpg')

instances = []
images_path='/home/majid/Projects/ML/IoT/bobcatscraper/images/converted-beforagument/'
# Load in the images
...
print(bobcats.shape)
bobcat_group=np.arange(2000).reshape(2000,1); 
bobcat_group.fill(10)
print(bobcat_group  .shape)
x_train=np.concatenate((x_train,bobcats[:1600,:,:,:]),axis=0)
x_test=np.concatenate((x_test,bobcats[1600:,:,:,:]),axis=0)
y_train=np.concatenate((y_train,bobcat_group[:1600]),axis=0)
y_test=np.concatenate((y_test,bobcat_group[1600:]),axis=0)
...
model = keras.models.load_model('/home/majid/Projects/ML/IoT/bobcatscraper/model.h5')
